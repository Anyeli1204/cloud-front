...
import { User, Hash, Globe, Calendar, Link as LinkIcon, Sliders, RefreshCw, Search } from "lucide-react";
...
export function FilterPanelDb({ onApply, onReset }: FilterPanelDbProps) {
	...
	const [activeTab, setActiveTab] = useState<'basic' | 'engagement' | 'advanced' | 'fecha'>('basic');
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const numericKeys: (keyof UserDBQueryRequest)[] = [
			"minViews", "maxViews", "minLikes", "maxLikes", "minEngagement", "maxEngagement", "minTotalInteractions", "maxTotalInteractions"
		];
		...
		<div className="w-full max-w-7xl mx-auto p-6">
			<div className="flex flex-col lg:flex-row gap-6">
				...
				<div className="flex-1 lg:flex-[3] h-full">
					<div ref={filterPanelRef} className="bg-white/80 backdrop-blur rounded-2xl shadow-lg pt-0 pb-6 px-6 min-h-[340px] h-full flex flex-col justify-center dark:bg-white/80 transition-all duration-500">
						<form onSubmit={handleSubmit} className="flex flex-col h-full">
							{/* Header: título y botones en la misma fila */}
							<div className="flex flex-col md:flex-row md:items-center md:justify-between mt-4 mb-6 gap-4">
								<h1 className="text-2xl md:text-3xl font-extrabold text-left bg-clip-text text-transparent bg-gradient-to-r from-purple-900 via-purple-800 to-purple-900" style={{ fontFamily: 'Nunito, sans-serif' }}>
							...
							<div className="flex w-full bg-gray-100 rounded-2xl p-1 gap-0 shadow-inner mb-6">
								...
								>
										<User className={`w-4 h-4 ${activeTab === 'basic' ? 'text-purple-500' : 'text-gray-400'}`} /> Basic Filters
									</button>
								<button
									type="button"
									...
									style={{ boxShadow: undefined }}
								>
										<Sliders className={`w-4 h-4 ${activeTab === 'engagement' ? 'text-purple-500' : 'text-gray-400'}`} /> Engagement
									</button>
								<button
									type="button"
									...
									style={{ boxShadow: undefined }}
								>
										<Globe className={`w-4 h-4 ${activeTab === 'advanced' ? 'text-purple-500' : 'text-gray-400'}`} /> Advanced
									</button>
								<button
									type="button"
									...
									style={{ boxShadow: undefined }}
								>
										<Calendar className={`w-4 h-4 ${activeTab === 'fecha' ? 'text-purple-500' : 'text-gray-400'}`} /> Fecha
									</button>
							</div>
							{/* Panel de campos */}
							<div className="flex flex-col gap-6 flex-1">
								{activeTab === 'basic' && (
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										{/* TikTok Usernames */}
										<div>
											<label className="block mb-1 font-medium text-gray-700">TikTok Usernames</label>
										...
										{/* Hashtags */}
										<div>
											<label className="block mb-1 font-medium text-gray-700">Hashtags</label>
											<div className="relative">
								...
								{activeTab === 'advanced' && (
									<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
										{/* Post ID */}
										<div>
											<label className="block mb-1 font-medium text-gray-700">Post ID</label>
											<div className="relative">
										...
										{/* Sound ID */}
										<div>
											<label className="block mb-1 font-medium text-gray-700">Sound ID</label>
											<div className="relative">
												<LinkIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
												<input
													type="text"
								...
								{activeTab === 'fecha' && (
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										{/* Fecha Desde */}
										<div>
											<label className="block mb-1 font-semibold text-gray-700">Fecha Desde <span className="text-purple-500">*</span></label>
											<div className="relative">
										...
										{/* Fecha Hasta */}
										<div>
											<label className="block mb-1 font-semibold text-gray-700">Fecha Hasta <span className="text-purple-500">*</span></label>
											<div className="relative">
												<Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
												<input
													type="date"
				...
				{/* Columna derecha: Carrusel de ayuda */}
				<div className="lg:w-80 lg:flex-shrink-0 h-full flex items-center justify-center">
					<div ref={helpPanelRef} style={{ height: helpHeight ? helpHeight : undefined, minHeight: '320px' }} className="relative w-full max-w-sm mx-auto bg-white/80 backdrop-blur rounded-2xl shadow-xl px-10 py-8 flex flex-col justify-center items-center transition-all duration-500">
						<DbHelpCarousel />
					</div>
				</div>
			</div>
