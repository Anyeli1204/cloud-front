...
const steps = [
  ...
  {
    title: "",
    content: "Usa los tabs para elegir el tipo de filtro: básicos, engagement, avanzados o por fecha",
    emoji: "❓"
  },
  {
    title: "Filtros básicos",
    content: "Filtra por usernames de TikTok o hashtags. Solo uno a la vez 🧑‍💻 #️⃣",
    emoji: "#️⃣"
  },
  {
    title: "Engagement",
    content: "Limita los resultados por vistas, likes, engagement o interacciones ",
    emoji: "📊"
  },
  {
    title: "Avanzado",
  ...
  {
    title: "Recuerda",
    content: "Haz clic en 'Aplicar' para ver los resultados. Usa 'Limpiar' para reiniciar los filtros",
    emoji: "✨"
  }
];
    ...
    <div className="w-full h-full flex flex-col justify-center items-center select-none transition-all duration-500">
      <div className="flex flex-1 w-full items-center justify-center relative">
        ...
        <div className="flex flex-col items-center justify-center w-full px-2">
          {steps[step].emoji && (
            <span className="text-4xl mb-2 animate-fade-in" style={{transitionDelay:'100ms'}}>{steps[step].emoji}</span>
          )}
          {steps[step].title && (
            <h2 className="font-extrabold text-center mb-2 text-purple-800 text-2xl md:text-3xl" style={{ fontFamily: 'Nunito, sans-serif' }}>{steps[step].title}</h2>
          )}
          {steps[step].content && (
            <p className="text-gray-700 text-center text-lg md:text-xl mb-2 max-w-xs md:max-w-sm" style={{lineHeight:'1.4'}}>{steps[step].content}</p>
          )}
        </div>
        {/* Flecha derecha */}
        <button
