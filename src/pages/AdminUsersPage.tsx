...
export default function AdminUsersPage() {
  ...
  const [sortBy, setSortBy] = useState<"username" | "email" | "role">("username");
  ...
  // Renderizado de paginación tipo ventana deslizante
  const renderPagination = () => {
    // Calcular el rango de páginas a mostrar
    let windowStart = pageWindowStart;
    let windowEnd = Math.min(windowStart + PAGE_WINDOW_SIZE - 1, totalPages);
    // Si la página actual está fuera de la ventana, ajusta la ventana
    if (page < windowStart) {
      windowStart = page;
  ...
  if (!loading && users.length === 0 && !error) {
    return <div className="p-8 text-center text-gray-500">
      No hay usuarios recibidos del backend.<br/>
      <pre className="text-xs text-left bg-gray-100 p-4 rounded mt-4 overflow-x-auto">{JSON.stringify(raw, null, 2)}</pre>
    </div>;
  }
      ...
      <div className="overflow-x-auto rounded-xl border border-purple-100 shadow">
        <table className="min-w-full text-sm">
          <thead className="bg-purple-50 dark:bg-neutral-800">
            <tr>
              <th className="p-3 cursor-pointer" onClick={() => {setSortBy("username"); setSortOrder(sortOrder === "asc" ? "desc" : "asc");}}>
                Usuario {sortBy === "username" && (sortOrder === "asc" ? "▲" : "▼")}
              </th>
              <th className="p-3 cursor-pointer" onClick={() => {setSortBy("email"); setSortOrder(sortOrder === "asc" ? "desc" : "asc");}}>
                Email {sortBy === "email" && (sortOrder === "asc" ? "▲" : "▼")}
              </th>
              <th className="p-3">Nombre</th>
              <th className="p-3">Apellido</th>
              <th className="p-3 cursor-pointer" onClick={() => {setSortBy("role"); setSortOrder(sortOrder === "asc" ? "desc" : "asc");}}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8"><Loader2 className="animate-spin mx-auto text-purple-500" size={32} /></td></tr>
            ) : pagedUsers.length === 0 ? (
                ...
                <tr key={user.id} className="border-b last:border-b-0 hover:bg-purple-50/40 dark:hover:bg-neutral-800/40 transition">
                  ...
                  <td className="p-3">
                    {user.role === "ADMIN" ? (
                      ...
                      >
                        {upgrading === user.id ? <Loader2 className="animate-spin" size={16} /> : <UserPlus size={16} />}
                        Hacer admin
                      </button>
                    )}
                  </td>
